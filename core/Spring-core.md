...
* 인터페이스로 초기화와 소멸전 콜백을 받는 방법을 바로 알아보자
  * NetworkClient 클래스에 InitializingBean, DisposableBean 인터페이스를 상속받으면 구현 메소드로    
    afterPropertiesSer(), destroy()를 구현한다. 이는 각각 의존관계 주입완료 이후, 빈 종료 직전에 호출되는   
    메소드 이다. 즉 초기화와 소멸을 지원해준다.

  * 실제 출력 결과도 초기화 메소드가 의존관계 주입 완료후에 호출되며, 스프링 컨테이너 종료가 호출되자   
    소멸 메소드가 호출 된 것도 알 수 있다.
<br><br>
* **초기화, 소멸 인터페이스 단점**
