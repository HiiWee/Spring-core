...
<br><br>
**모든 메소드의 호출 시간을 측정하고 싶다면 (MemberService)**
* 시작 시간과 끝날때의 시간을 계산하는 로직을 각 메소드의 개수만큼 작성해야한다.
* 하지만 위와같이 실제 코드를 작성하게 된다면 여러 문제점이 보인다.
  * 회원가입, 회원 조회에 시간을 측정하는 기능은 핵심 관심 사항이 아니다.(핵심 비즈니스 로직이 아님)
  * 시간을 측정하는 로직은 공통 관심 사항이다.
  * 시간 측정 로직과 핵심 비즈니스 로직이 섞여 유지보수 어려움
  * 시간 측정 로직을 별도의 공통 로직으로 만들기 어렵다(템플릿 메소드 패턴 요구됨)
  * 혹여나 시간 측정 로직이 변경되면 모든 로직을 변경해야하는 비효율적 작업 요구
* 위와 같은 상황을 해결해 주는것이 `AOP`이다.
<br><br>
* 여기서 기존 회원가입, 회원 조회 같은 핵심 비즈니스 로직을 핵심 관심 사항(core concern)이라 말한다.
* 각 메소드의 시간 측정과 같은 곁다리 업무들을 공통 관심 사항 (cross-cutting concern)이라 말한다.
